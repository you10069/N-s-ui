...
func (s *SubService) GetSubs(subId string) (*string, []string, error) {
	...
	db := database.GetDB()
	client := &model.Client{}
	err = db.Model(model.Client{}).Where("enable = true and blocked = false and name = ?", subId).First(client).Error
	if err != nil {
		return nil, nil, err
	}
...
func (s *SubService) getClientInfo(c *model.Client) string {
	...
	adjusted := int64(float64(c.Up+c.Down) * multiplier)
	if vol := c.Volume - adjusted; c.Volume > 0 && vol > 0 {
